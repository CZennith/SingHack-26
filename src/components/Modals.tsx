...
import { ClientSnapshot } from '../services/apiClient';
      ...
      <div className="bg-white border border-[#dedbd5] max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl flex flex-col">
        ...
        <div className="p-6 space-y-5 text-[13px] leading-relaxed text-[#1e1e1e]">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[#121212] font-medium">{client.headlineIssue || 'No current mandate strain is calculated.'}</p>
            <p className="text-[#555555]">{client.summary || 'Narrative client summaries are not calculated by the current backend.'}</p>
          </div>
          ...
          <div className="p-3.5 bg-[#fdf8f0] border border-[#f4e4cc] text-[12px]">
            ...
            <p className="text-[#121212]">{client.suggestedNextStep || 'No recommendation is generated by the current backend.'}</p>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[12px]">
              <div className="p-3 bg-[#faf9f6] border border-[#e8e5e0]">
                ...
                <ul className="space-y-1 text-[#555555]">
                  {client.strategicMatrix.risks.length ? client.strategicMatrix.risks.slice(0, 2).map((r, i) => (
                    <li key={i} className="list-disc ml-3 text-[11.5px]">
                      {r.title}
                    </li>
                  )) : <li className="text-[11.5px] text-[#888888]">Not calculated</li>}
                </ul>
              </div>
              ...
              <div className="p-3 bg-[#faf9f6] border border-[#e8e5e0]">
                ...
                <ul className="space-y-1 text-[#555555]">
                  {client.strategicMatrix.opportunities.length ? client.strategicMatrix.opportunities.slice(0, 2).map((o, i) => (
                    <li key={i} className="list-disc ml-3 text-[11.5px]">
                      {o.title}
                    </li>
                  )) : <li className="text-[11.5px] text-[#888888]">Not calculated</li>}
...
export const SourceDataModal: React.FC<SourceDataModalProps> = ({ isOpen, onClose, snapshot }) => {
      ...
      <div className="bg-white border border-[#dedbd5] max-w-lg w-full shadow-2xl p-6 space-y-5">
        <div className="flex items-center justify-between border-b border-[#e8e5e0] pb-3">
          <div>
            ...
            <h3 className="font-serif text-[20px] text-[#121212]">{snapshot ? 'Connected Source Data' : 'Planned Data Sources'}</h3>
          </div>
        ...
        {snapshot ? (
          <div className="space-y-3 font-mono text-[11.5px] text-[#555555]">
            <div className="p-3 bg-[#faf9f6] border border-[#e8e5e0]">
              <div className="font-medium text-[#121212]">Read-only client snapshot</div>
              <div className="text-[10px] text-[#888888] mt-1">
                {snapshot.snapshot_metadata.client_id} · {snapshot.snapshot_metadata.as_of_date} · calculation {snapshot.snapshot_metadata.calculation_version}
              </div>
            </div>
            <div className="grid grid-cols-2 gap-2 text-[10.5px]">
              {([
                ['Portfolios', snapshot.portfolios.length], ['Holdings', snapshot.holdings.length],
                ['Transactions', snapshot.transactions.length], ['Facilities', snapshot.credit_facilities.length],
                ['Commitments', snapshot.commitments.length], ['RM notes', snapshot.rm_notes.length],
                ['Events', snapshot.market_events.length], ['Source references', snapshot.source_references.length],
              ] as const).map(([label, count]) => (
                <div key={label} className="p-2 bg-[#faf9f6] border border-[#e8e5e0] flex justify-between">
                  <span>{label}</span><strong className="text-[#121212]">{count}</strong>
                </div>
              ))}
            </div>
            <p className="text-[10.5px] text-[#888888]">The browser receives validated, client-scoped snapshot records; it never opens DuckDB.</p>
          </div>
        ) : <div className="space-y-3 font-mono text-[11.5px] text-[#555555]">
